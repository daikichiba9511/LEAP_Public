...
006
    ...
    + in_chans=(25,64,128,256,512,1024) (078)
"""
...
class Config(pydantic.BaseModel):
    ...
    seed: int = pydantic.Field(default=42, description="seed")
    # -- Model
    unet_in_chans: tuple[int, int, int, int, int, int] = pydantic.Field(
        default_factory=lambda: (25, 64, 128, 256, 512, 1024)
    )
    ...
    n_rows: int = pydantic.Field(default=1_000_000 * 6, description="number of rows to load")
    ...
    train_batch_size: int = pydantic.Field(default=1024 * 4, description="train batch size")

    # -- Valid
    valid_batch_size: int = pydantic.Field(default=1024 * 4, description="valid batch size")
...
class LeapDataset(torch_data.Dataset):
    def __init__(
        ...
        x = torch.from_numpy(data[x_features].to_numpy().astype(np.float64))
        y = torch.from_numpy(data[y_features].to_numpy().astype(np.float64))
        self.x = x
        ...
        logger.info(f"{self.x.shape = } {self.y.shape = }, {self.x.dtype = } {self.y.dtype = }")
...
def _load_train_valid_data(
    ...
    if file_path.suffix == ".parquet":
        data = pl.read_parquet(file_path, n_rows=n_rows)
...
def init_dataloader(
    ...
    calc_mean_std(train_ds.x, train_ds.y)  # type: ignore
