...
006
    ...
    + use train_sampled_8000000.parquet (082)
    + huver loss (083)
    + huver looss + mse loss
"""
...
class Config(pydantic.BaseModel):
    ...
    seed: int = pydantic.Field(default=1994, description="seed")
    # -- Model
...
def main() -> None:
    ...
    for fold in range(cfg.n_fold):
        ...
        def criterion_train(y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
            return 0.5 * nn.MSELoss()(y_pred, y_true) + 0.5 * nn.HuberLoss(delta=1.0)(y_pred, y_true)
