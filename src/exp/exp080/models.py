...
class TransformerMiddle(nn.Module):
    def __init__(
        self,
        d_model: int = 4,
        nhead: int = 2,
        ...
        super().__init__()
        self.conv1d = nn.Conv1d(256, 512, kernel_size=3, stride=1, padding="same")
        self.transformer_encoder = nn.TransformerEncoder(
            encoder_layer=nn.TransformerEncoderLayer(
                d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward, dropout=0.0, activation="relu"
            ),
            num_layers=1,
        )
...
if __name__ == "__main__":
    ...
    in_chans = (25, 32, 64, 128, 256, 512)
    # in_chans = (25, 64, 128, 256, 512, 1024)
    model = LeapUnet1D(in_chans).cpu()
    print(model)
    print(summary(model, (8, 556), device="cpu"))
    #
